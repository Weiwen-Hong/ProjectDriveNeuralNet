...
class Game(Widget):
    ...
    def update(self):
        ...
        last_signal.append(self.car.angle % 360)
        last_signal.append(-1 * (self.car.angle % 360))
	...
	print(last_signal)
        ...
        if sand[self.car.x + 6 * math.cos((rotation + self.car.angle) / 180.0)][self.car.y + 6 * math.cos((rotation + self.car.angle) / 180.0)] > 0:
            last_reward = -1
            print("on sand")
	    ...
	    if self.car.x < 15:
	         self.car.x = 15
                 last_reward = -1.6
	    if self.car.x > self.width - 15:
		self.car.x = self.width - 15
		last_reward = -1.6
	    if self.car.y < 15:
		self.car.y = 15
		last_reward = -1.6
            if self.car.y > self.height - 15:
